...
class Huk : public Entity {
    Sprite spr = Sprite(Vec2i(64, 224), Vec2i(32,32), Vec2i(g_gm.vu(2), g_gm.vu(2)));
...
public:
    ...
    void process() override {
        ...
        if (!hasShown && g_gm.getPuzzleBits() == 3 && g_gm.getPuzzleChallenge() == 'e') {
            show = true;
            hasShown = true;
            g_gm.setHukActive(true);
            sndNext.play();
            if (g_gm.getPuzzleNum() == 0) {
        ...
        if (!hasShown && g_gm.getPuzzleBits() == 3 && g_gm.getPuzzleChallenge() == 'h') {
            if (g_gm.getPuzzleNum() == 69) {
                show = true;
                hasShown = true;
                g_gm.setHukActive(true);
                sndNext.play();
    ...
    void render(Graphics* graph) override {
        if (!show) return;
        int topY = WINDOW_SIZE.y - g_gm.vu(8);
        graph->setColor(g_colors["GRAY"]);
        graph->rect(Vec2i(0 + g_gm.vu(0.25f), topY), Vec2i(WINDOW_SIZE.x - g_gm.vu(0.5f), g_gm.vu(4)));
        graph->setColor(g_colors["BG3"]);
        float inOff = g_gm.vu(0.1f);
        float inOff2 = inOff * 2;
        graph->rect(Vec2i(inOff + g_gm.vu(0.25f), topY + inOff), Vec2i(WINDOW_SIZE.x - inOff2 - g_gm.vu(0.5f), g_gm.vu(4) - inOff2));
        
        graph->setColor(g_colors["BLUE"]);
        inOff = g_gm.vu(0.25f) + g_gm.vu(0.01f * ((g_gm.getTick() / 2) % 16));
        inOff2 = inOff * 2;
        graph->rect(Vec2i(inOff, topY + inOff), Vec2i(g_gm.vu(2) - inOff2, g_gm.vu(2) - inOff2));
        ...
        graph->textFmt(messages[messageIndex], Vec2i(g_gm.vu(2), topY + g_gm.vu(0.5f)), &Fonts::medium, WINDOW_SIZE.x - g_gm.vu(2.5f));
    }
private:
