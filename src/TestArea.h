...
public:
  ...
  void reset() {
    ...
    case 'e':
      ...
      break;
    case 'm':
      testCount = Util::maxUnsignedInt(_g.getPuzzleBits()) / 2;
      break;
    case 'h':
      testCount = Util::maxUnsignedInt(_g.getPuzzleBits());
      break;
    default:
      testCount = Util::maxUnsignedInt(_g.getPuzzleBits());
      break;
  ...
  void process() {
    if (_g.getScreen() != SCN_PUZZLE)
      return;
    em.checkMouse();
    em.process();
    std::string cs = StringTools::replace(
        StringTools::replace(_g.getCodeString(), " ", ""), "_", "");
    ...
    // XXX - Maybe slow
    pos.x = WINDOW_SIZE.x -
            (_g.getShowTests() ? testWinWidth : _g.cellSize * 0.75f);
    for (int i = 0; i < tests.size(); i++) {
  ...
  void render(Graphics *graph) {
    ...
    if (_g.hasCodeErr()) {
      ...
      graph->text("!! ERROR",
                  Vec2i(pos.x + _g.vu(0.5f), pos.y - (_g.getTick() / 4 % 8)),
                  &Fonts::medium);
    } else {
      graph->setColor(_colors["GRAY"]);
      graph->text(_g.getPuzzleString(),
                  Vec2i(pos.x + _g.vu(1.1f), pos.y + _g.vu(0.25f)),
                  &Fonts::medium);
    }
    if (testFails == 0) {
