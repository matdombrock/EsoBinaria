...
// DEBUG
//
inline void DBG(const std::string &value) { std::cout << value << std::endl; }
inline void DBG(const char *value) { std::cout << value << std::endl; }
inline void DBG(int value) { std::cout << value << std::endl; }
inline void DBG(float value) { std::cout << value << std::endl; }
...
public:
  ...
  Vec2 operator*(T scalar) const { return Vec2<T>(x * scalar, y * scalar); }
  ...
  Vec2 floor() const { return Vec2<T>(std::floor(x), std::floor(y)); }
  T size2d() const { return x * y; }
  Vec2 copy() const { return Vec2<T>(x, y); }
...
public:
  ...
  static std::string replace(const std::string &str, const std::string &find,
  ...
  static std::vector<std::string> splitSketch(const std::string &str,
                                              const std::string &delimiter,
                                              bool cleanDelimiter = true) {
...
public:
  ...
  bool hasKey(std::string key) { return data.find(key) != data.end(); }
  void clear() {
    data.clear();
    write();
  }
...
public:
  ...
  void poll() {
    for (int i = 0; i < 512; i++) {
      if (keyStatePrev[i] != keyState[i])
        DBG("Key changed " + std::to_string(i) + " " +
            std::to_string(keyState[i]));
  ...
  bool mouseKey(Uint32 key) { return mouseState == key; }
  ...
  bool anyKeyOnce() {
    ...
    if (mouseState && (mouseStatePrev != mouseState))
      return true;
    return false;
  }
  Vec2i wasd() {
...
public:
  ScreenFX() {}
  ~ScreenFX() {}
  void render(SDL_Renderer *renderer, int tick, float mix, float p1, float p2,
              float p3) {
...
protected:
  virtual void renderPixel(Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a,
                           ScreenFXProps props) {}
  int width, height;
...
protected:
  void renderPixel(Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a,
                   ScreenFXProps props) override {
    *r = 255 - *r;
    *g = 255 - *g;
    *b = 255 - *b;
  }
...
protected:
  void renderPixel(Uint8 *r, Uint8 *g, Uint8 *b, Uint8 *a,
    ...
    if (mod % scanSize >= scanSize - subScanSize) {
      Uint32 pixel =
          props.pixels[props.y * width + props.x + (mod % subScanSize)];
      Uint8 r1, g1, b1, a1;
...
public:
  ...
  void sprite(Vec2i sheetPos, Vec2i pos, Vec2i spriteSize, Vec2i size,
    ...
    SDL_RenderCopyEx(renderer, spritesheet, &srcRect, &dstRect, 0, nullptr,
                     flip);
  }
  ...
  void text(const std::string &text, Vec2i pos, Font *font) {
    ...
    SDL_Surface *surface =
        TTF_RenderText_Solid(font->data, text.c_str(), color);
    if (surface == nullptr) {
  ...
  void textFmt(const std::string &text, Vec2i pos, Font *font,
               int maxWidth = 1024) {
    // Split by new lines
    ...
    for (std::string line : linesOverflow) {
      std::vector<std::string> subChunks =
          StringTools::splitSketch(line, "<$", false);
      ...
      for (std::string subChunk : chunk) {
        if (subChunk[0] == '<' && subChunk[1] == '$') {
          if (subChunk.length() >= 11) {
            ...
            Color color = Color(std::stoi(colorCode.substr(0, 2), nullptr, 16),
  ...
  void circle(Vec2i center, int radius, bool fill = true) {
    ...
    while (offsetY >= offsetX) {
      if (fill) {
        status +=
            SDL_RenderDrawLine(renderer, center.x - offsetY, center.y + offsetX,
                               center.x + offsetY, center.y + offsetX);
        status +=
            SDL_RenderDrawLine(renderer, center.x - offsetX, center.y + offsetY,
        ...
        status += SDL_RenderDrawPoint(renderer, center.x + offsetX,
                                      center.y + offsetY);
        status += SDL_RenderDrawPoint(renderer, center.x + offsetY,
                                      center.y + offsetX);
        status += SDL_RenderDrawPoint(renderer, center.x - offsetX,
                                      center.y + offsetY);
        status += SDL_RenderDrawPoint(renderer, center.x - offsetY,
  ...
  void line(Vec2i start, Vec2i end, int thickness = 1) {
    if (thickness <= 1) {
      ...
      verts[3].position.x = p4.x;
      verts[3].position.y = p4.y;
  ...
  void point(Vec2i pos) { SDL_RenderDrawPoint(renderer, pos.x, pos.y); }
  ...
  Vec2i getWindowSize() { return windowSize; }
  ...
  // WARN: This is a bad idea
  void fxApply(FXName name, int tick, float mix = 1.0f, float p1 = 0.0f,
...
public:
  ...
  void set(std::string path) {
    ...
    //     Mix_GetError() << std::endl; DBG("Cant initialize SDL_mixer");
    //     exit(1);
    // }
  ...
  void mute() { Mix_Volume(channel, 0); }
  void unmute() { Mix_Volume(channel, volume); }
  ...
  bool isPlaying() { return Mix_Playing(channel); }
...
public:
  ...
  Vec2i spriteSize;
  int fps = 0;
  Sprite(Vec2i sheetPos = Vec2i(0, 0), Vec2i spriteSize = Vec2i(16, 16),
  ...
  void setSheetPos(Vec2i sheetPos) { this->sheetPos = sheetPos; }
  void setAnimation(std::vector<Vec2i> frames, int fps = 1,
                    bool pingPong = false) {
    if (pingPong) {
  ...
  void stopAnimation() { isAnimated = false; }
  void render(Graphics *graph, Vec2i pos, bool flipX = false,
              bool flipY = false) {
...
public:
  ...
  Entity() { collider = new BoxCollider(pos, Vec2i(0, 0)); }
  ~Entity() {}
  // XXX - Pointless?
  void init(int id) { onStart(); }
  int getId() { return id; }
  ...
  void updateCollider() { collider->pos = pos; }
  void drawCollider(Graphics *graph) {
    graph->setColor(0, 255, 0);
    graph->rect(collider->pos + collider->positionOffset, collider->size,
                false);
  }
  // Called after init
...
public:
  ...
  void render(Graphics *graph) {
    ...
    std::sort(entities.begin(), entities.end(),
              [](Entity *a, Entity *b) { return a->zindex < b->zindex; });
...
public:
  ...
  void render(Graphics *graph) override {
    ...
    if (center) {
      ...
      Vec2i textPos =
          pos + Vec2i((size.x - textWidth) / 2, (size.y - font.size) / 2);
...
public:
  ...
  static int maxUnsignedInt(int bits) { return (1 << bits); }
};
...
public:
  ...
  Main(const char *windowTitle = "Imp", Vec2i windowSize = {800, 600},
       int fps = 60, int scale = 1, std::string spriteSheet = "")
      : fps(fps), windowTitle(windowTitle), windowSize(windowSize),
  ...
  int getRealFPS() { return 1000 / frameTimeAvg; }
